...
contract EventTickets {
  ...
  */
  event LogBuyTickets(address purchaser, uint purchased);
  event LogGetRefund(address requester, uint refunded);
  ...
  */
  ...
  modifier paidEnough(uint numTickets) {
    require(msg.value >= numTickets * TICKET_PRICE, 'You did not pay enough');

    _;
  }

  modifier ticketsAvailable(uint numTickets) {
  ...
  */
  function getBuyerTicketCount(address _address) public view returns (uint tickets) {
  ...
  */
  function buyTickets(uint _numTickets)
  public
  payable
  isOpenMod()
  paidEnough(_numTickets)
  ticketsAvailable(_numTickets)
  checkValue(_numTickets)
{
  ...
  /*
    ...
    TODO:
      ...
      - Emit the appropriate event.
  */
  function getRefund() public hasPurchased() {
    uint numTickets = myEvent.buyers[msg.sender];

    myEvent.sales -= numTickets;

    msg.sender.transfer(TICKET_PRICE * numTickets);
